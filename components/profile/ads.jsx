...
const AdsComponent = () => {
  const [ads, setAds] = useState([
    { id: 1, title: 'Ad Title 1', region: 'Region A', sponsored: false, imageUrl: 'https://via.placeholder.com/150' },
    { id: 2, title: 'Ad Title 2', region: 'Region B', sponsored: true, imageUrl: 'https://via.placeholder.com/150' },
    { id: 3, title: 'Ad Title 3', region: 'Region C', sponsored: false, imageUrl: 'https://via.placeholder.com/150' },
    ...
    const updatedAds = ads.map(ad => {
      if (ad.id === id) {
        ...
        if (updatedAd.sponsored) {
          ...
          const isAnotherSponsored = ads.some(ad => ad.id !== id && ad.sponsored);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {ads.map(ad => (
          <div key={ad.id} className="ad-card bg-white rounded-lg shadow-md">
            <img src={ad.imageUrl} alt={ad.title} className=" h-60 w-full object-cover" />
            <div className='p-4'>
            <h3 className="text-xl font-semibold">{ad.title}</h3>
            <p className="mb-2">Region: {ad.region}</p>
            <p className="mb-4">Sponsored: {ad.sponsored ? 'Yes' : 'No'}</p>
            <div className="flex justify-between items-center">
              <button onClick={() => handleSponsoredToggle(ad.id)} className={`px-4 py-2 ${ad.sponsored ? 'bg-red-500' : 'bg-blue-500'} text-white rounded-md hover:bg-blue-600`}>
                {ad.sponsored ? 'Sponsored' : 'Make Sponsored'}
              </button>
              <select value={ad.region} onChange={(e) => handleRegionSelect(ad.id, e.target.value)} className="px-4 py-2 bg-gray-200 rounded-md focus:outline-none">
