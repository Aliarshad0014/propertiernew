        ...
        <div className="mb-4">
          <div className="relative">
            ...
            <div className="absolute top-0 right-0 flex justify-end mt-2 mr-4 space-x-2">
              <button className="bg-yellow-500 hover:bg-yellow-600 transition-all text-white px-4 py-2 rounded">Support</button>
              <button className="bg-red-500 hover:bg-red-600 transition-all text-white px-4 py-2 rounded">Logout</button>
              <button className="bg-gray-500 hover:bg-gray-600 transition-all text-white px-4 py-2 rounded">Edit Cover Photo</button>
          ...
          <div className="w-full lg:w-1/3 bg-gray-50 shadow-lg flex flex-col items-center p-8 min-h-screen">
            <div className="sticky top-0 pt-8 mb-4">
              <div className="flex justify-between">
                ...
                />
              <button className="text-red-500 flex top-0 hover:text-black focus:outline-none" onClick={handleNotifications}>
                <FaBell size={24} />
              </button>
              </div>
              <h2 className="text-xl font-bold mt-2 text-start">{userData.name}</h2>
              <div className="mt-2">
                <button className="bg-yellow-500 hover:bg-black transition-all text-white px-4 py-2 rounded w-full lg:w-auto">Edit Your Profile</button>

              </div>
              <div className="mt-4 text-left bg-gray-100 font-light shadow-lg rounded-md space-y-2 p-4 lg:p-2">
                <p><strong>Email:</strong> {userData.email}</p>
                <p><strong>Phone Number:</strong> {userData.phoneNumber}</p>
                <p className='hover:underline cursor-pointer text-green-500'><strong>Following:</strong> {userData.following}</p>
                <p className='hover:underline cursor-pointer text-green-500'><strong>Followers:</strong> {userData.followers}</p>
                <p><strong>Total Ads:</strong> {userData.totalAds}</p>
                <p><strong>Appreciations:</strong> {userData.appreciations}</p>
                <p><strong>Address:</strong> {userData.address}</p>
                <p><strong>Since:</strong> {userData.since}</p>
              </div>
              <div className="mt-4 flex flex-wrap justify-center lg:justify-start">
                <button className="bg-yellow-500 hover:bg-black transition-all text-white px-4 py-2 rounded mr-2 mt-2">Manage HRM</button>
                <button className="bg-yellow-500 hover:bg-black transition-all text-white px-4 py-2 rounded mr-2 mt-2">Manage CRM</button>
                <button className="bg-yellow-500 hover:bg-black transition-all text-white px-4 py-2 rounded mt-2 mr-2">Manage ERP</button>
                <button className="bg-black hover:bg-yellow-500 transition-all text-white px-4 py-2 rounded mt-2">Your Website</button>
              </div>
            </div>
          </div>
          ...
          <div className="w-full lg:w-2/3 p-6 bg-white shadow-xl overflow-y-auto">
            <div className="flex mb-4 flex-wrap">
              ...
              {Object.keys(sectionContent).map((section) => (
                <button
                  key={section}
                  className={`text-md font-regular mr-8 mb-4 ${activeSection === section ? 'font-bold text-yellow-500' : 'text-black'}`}
