  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <h2 className="text-2xl font-semibold mb-6 text-yellow-500 mt-10">Recent Blogs</h2>
              ...
              <Link href={`/blogpages/${mainBlog.id}`}>
                <div className="h-80 w-full relative cursor-pointer hover:shadow-lg">
                  ...
                  <div className="absolute bottom-0 w-full bg-gray-700 bg-opacity-30 p-4">
                    <h2 className="text-xl font-semibold text-white">{mainBlog.title}</h2>
                    <p className="text-sm text-gray-300">{formatDate(mainBlog.published_date)}</p>
                  </div>
                </div>
              </Link>
            )}
              ...
              <Link key={index} href={`/blogpages/${pick.id}`}>
                <div className="relative cursor-pointer h-full hover:shadow-lg transition-all">
                  ...
                  <div className="absolute bottom-0 w-full bg-gray-700 bg-opacity-30 p-2 group-hover:bg-opacity-50 transition-all">
                    <h3 className="lg:text-sm text-xs font-semibold text-white truncate">{pick.title}</h3>
                    <p className="lg:text-sm text-xs text-gray-300">{formatDate(pick.published_date)}</p>
                  </div>
                </div>
              </Link>
            ))}
        ...
        <h2 className="text-2xl font-semibold mb-6 text-yellow-500 mt-10">Latest Blogs</h2>
        ...
        <div className=" flex flex-col lg:flex-row gap-4">
            ...
            {blogs.map((post) => (
              <Link key={post.id} href={`/blogpages/${post.id}`}>
                <div className="relative cursor-pointer ">
                  ...
                  <div className="pt-4">
                    <p className="text-xs font-medium text-yellow-500">{formatDate(post.published_date)}</p>
                    <h3 className="text-base font-semibold mt-2 text-gray-700">{post.title}</h3>
                  </div>
                </div>
              </Link>
          ...
          <div className="w-full lg:w-1/4 p-4 bg-gray-100 text-black">
            <h2 className="text-xl font-semibold mb-4 text-gray-700">Advertisements</h2>
          </div>
        </div>
      </div>
      <div className='py-10'>
